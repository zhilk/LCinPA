...
addOptional(p, 'bxfacealpha', 1, @isnumeric)
...
bxfacealpha         = p.Results.bxfacealpha;
...
if box_on
    ...
    % 'box' of 'boxplot' — use patch instead of rectangle for FaceAlpha
    bx = box_pos(1); by = box_pos(2); bw = box_pos(3); bh = box_pos(4);
    px = [bx, bx+bw, bx+bw, bx];
    py = [by, by, by+bh, by+bh];
    h{3} = patch(px, py, bxfacecl, 'EdgeColor', bxcl, ...
        'LineWidth', line_width, 'FaceAlpha', bxfacealpha);
